...
[ApiController]
[Route("api/receipt-from-production")]
[Authorize]
public class ReceiptFromProductionController(
    ReceiptFromProductionService service,
    ...
    IPdfService pdfService) : ControllerBase
{
    ...
    public async Task<IActionResult> DownloadPdf(int id, CancellationToken cancellationToken)
    {
        ...
        var placeholders = new Dictionary<string, string>
        {
            ...
            ["warehouse"] = order?.Warehouse ?? lines.LastOrDefault()?.Warehouse ?? string.Empty,
        ...
        var pdfBytes = await pdfService.GeneratePdfFromTemplateAsync(
            "receipt-from-production-template.html", placeholders, cancellationToken);

        var fileName = $"ReceiptFromProduction({record.Id}).pdf";
