...
/// <summary>
/// Hangfire job that fully syncs purchase requests for one company DB, renewing the SAP session
...
/// </summary>
public class PurchaseRequestSyncJob(
...
{
    public const string JobName = "purchase-request-full-sync";

    /// <summary>
    /// One full sync at a time per companyDb argument (Hangfire lock key includes args).
    /// Concurrent enqueue is also guarded via <see cref="PurchaseRequestLocalStore.TryBeginFullSyncJobAsync"/>.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(companyDb))
            throw new InvalidOperationException("companyDb is required for purchase request full sync.");
