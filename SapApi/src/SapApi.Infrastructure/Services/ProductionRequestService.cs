...
public static class ProductionRequestMapper
{
    ...
    public static IssueForProductionRequests ToIssueEntity(SapInventoryGenExitRequestOrderLines orderLines)
    {
    ...
    public static ReceiptFromProductionRequests ToReceiptEntity(SapInventoryGenExitRequestOrderLines orderLines)
    {
        var po = orderLines.ProductionOrder
            ?? throw new ArgumentException("Production order is required.");
...
public sealed class IssueForProductionService(AppDbContext db)
{
        ...
        CancellationToken cancellationToken)
    {
        if (id is null or <= 0)
        {
            var entity = ProductionRequestMapper.ToIssueEntity(orderLines);
        ...
        var mapped = ProductionRequestMapper.ToIssueEntity(orderLines);
    ...
    {
        var existing = await db.IssueForProductionRequests.FindAsync([id], cancellationToken);
...
public sealed class ReceiptFromProductionService(AppDbContext db)
{
        ...
        CancellationToken cancellationToken)
    {
        if (id is null or <= 0)
        {
            var entity = ProductionRequestMapper.ToReceiptEntity(orderLines);
        ...
        var mapped = ProductionRequestMapper.ToReceiptEntity(orderLines);
        existing.RequestBody = mapped.RequestBody;
        existing.CardCode = mapped.CardCode;
        existing.CardName = mapped.CardName;
        existing.Project = mapped.Project;
    ...
    public async Task<bool> DeleteAsync(int id, CancellationToken cancellationToken)
    {
        var existing = await db.ReceiptFromProductionRequests.FindAsync([id], cancellationToken);
        if (existing is null) return false;
