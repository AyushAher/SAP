...
public class SapLoginServiceTests
{
    ...
    public async Task GetSessionIdAsync_NoAuthenticatedUser_ReturnsNull()
    {
        ...
        _sessionStore.Verify(s => s.GetSessionAsync(It.IsAny<int>(), It.IsAny<SapCompanyDatabase>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Test]
    public async Task GetSessionIdAsync_ValidSession_ReturnsSessionId()
    {
        SetUser(15, TestCompanyDb);
        _sessionStore
            .Setup(s => s.GetSessionAsync(15, TestCompanyDb, It.IsAny<CancellationToken>()))
    ...
    {
        SetUser(22, TestCompanyDb);
        _sessionStore
            .Setup(s => s.GetSessionAsync(22, TestCompanyDb, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SapSessionInfo
            {
        ...
        sessionId.Should().BeNull();
        _sessionStore.Verify(s => s.RemoveSessionAsync(22, TestCompanyDb, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Test]
    public async Task SapLoginAsync_ValidSession_DoesNotRenew()
    {
        SetUser(8, TestCompanyDb);
        _sessionStore
            .Setup(s => s.GetSessionAsync(8, TestCompanyDb, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SapSessionInfo
            {
                SessionId = "active",
                UserName = "sap-user",
        ...
        _sessionStore.Verify(s => s.GetCredentialsAsync(It.IsAny<int>(), It.IsAny<SapCompanyDatabase>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Test]
    public async Task LogoutAsync_RemovesStoredSession()
    {
        SetUser(3, TestCompanyDb);
        _sessionStore
            .Setup(s => s.GetSessionAsync(3, TestCompanyDb, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SapSessionInfo
            {
                SessionId = "logout-me",
                UserName = "sap-user",
        ...
        _sessionStore.Verify(s => s.RemoveSessionAsync(3, TestCompanyDb, It.IsAny<CancellationToken>()), Times.Once);
    }

    private void SetUser(int userId, SapCompanyDatabase companyDb)
    {
