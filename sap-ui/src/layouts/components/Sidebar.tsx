...
const approvalItems = [
  { to: ROUTES.APPROVALS, label: 'My Pending Approvals', icon: CheckSquare },
  { to: ROUTES.MY_APPROVAL_REQUESTS, label: 'Approval Status Report', icon: ClipboardList },
]
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-3 scrollbar-thin">
          ...
          {!collapsed && (
            <p className="px-3 pt-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
              Approval
            </p>
          )}
          {showAdmin && renderLink({
            to: ROUTES.APPROVAL_POLICIES,
            label: 'Approval Policies',
            icon: Shield,
          })}
          {approvalItems.map(renderLink)}
          {showAdmin && (
            <>
